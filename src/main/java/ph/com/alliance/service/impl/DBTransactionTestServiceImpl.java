...
public class DBTransactionTestServiceImpl implements DBTransactionTestService {
	...
	public boolean createTask(Task task) {
		...
		em.getTransaction().begin();
		try {
			result = taskDao.createTask(em, task);
			em.getTransaction().commit();
		} catch (Exception e) {
			e.printStackTrace();
			result = false;
	...
	public List<Task> getTasks() {
		...
		try {
			taskList = taskDao.getTaskList(em);
		} catch (Exception e) {
			System.out.print(e.getMessage());
		} finally {
			if (em.isOpen()) {
	...
	public List<Task> getTasksToDo() {
		...
		try {
			taskList = taskDao.getTaskListToDo(em);
		} catch (Exception e) {
			System.out.print(e.getMessage());
		} finally {
			if (em.isOpen()) {
	...
	public List<Task> getTasksInProgress() {
		...
		try {
			taskList = taskDao.getTaskListInProgress(em);
		} catch (Exception e) {
			System.out.print(e.getMessage());
		} finally {
			if (em.isOpen()) {
	...
	public List<Task> getTasksDone() {
		...
		try {
			taskList = taskDao.getTaskListDone(em);
		} catch (Exception e) {
			System.out.print(e.getMessage());
		} finally {
			if (em.isOpen()) {
...
// TODO Auto-generated method stub
			...
			try {
				taskList = taskDao.getTaskDetails(em, id);
			} catch (Exception e) {
				System.out.print(e.getMessage());
			} finally {
				if (em.isOpen()) {
	...
	@Override
	public Task editTask(Task ptask) {
		EntityManager em = transactionManager.getEntityManagerFactory().createEntityManager();
		Task task = null;
		...
		try {
			task = taskDao.updateTask(em, ptask);
			em.getTransaction().commit();
		} catch (Exception e) {
			e.getMessage();
			if (em.getTransaction().isActive()) {
